...
class AdminPage(tk.Frame):
    ...
    def display_order_information(self, name, phone, email, clothes, total_price):
        self.report_table = ttk.Treeview(self, columns=("Name", "Phone", "Email", "Cloth", "Laundry", "Total Price"), show="headings")
        ...
        self.report_table.insert("", "end", values=(name, phone, email, "", "", ""))  # Insert user info row
    ...
    def load_data(self):
        ...
        data_table = ttk.Treeview(data_window, columns=("ID", "Username", "Email"), show="headings")
        data_table.heading("ID", text="ID")
        data_table.heading("Username", text="Username")
        data_table.heading("Email", text="Email")

        for user in users:
            data_table.insert("", "end", values=user)
    ...
    def update_status(self):
        ...
        name, email, cloth, laundry, cost, status = self.report_table.item(selected_item)["values"][0], self.report_table.item(selected_item)["values"][1], self.report_table.item(selected_item)["values"][2], self.report_table.item(selected_item)["values"][3], self.report_table.item(selected_item)["values"][4], self.report_table.item(selected_item)["values"][5]      
